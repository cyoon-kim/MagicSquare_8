...
|---|---|---|---|
| `BlankFinder.find()` | Domain | FR-02 | `domain.blank_finder.BlankFinder.find` |
| `MissingNumberFinder.find()` | Domain | FR-03 | `domain.missing_number_finder.MissingNumberFinder.find` |
| `MagicSquareValidator.is_valid()` | Domain | FR-04 | `domain.magic_square_validator.MagicSquareValidator.is_valid` |
| `Solver.solve()` | Domain | FR-05a | `domain.solver.Solver.solve` |
...
| `src/domain/*` | 0% 호출 (spy) | AC-01 범위 — Domain 코드 미실행 |
...
| Domain | `--cov=src/domain --cov-fail-under=95` | 95% |
